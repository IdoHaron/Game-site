...
let cube_reloads = {number: 0, user_stack: [] };
//#region file_manage
app.set('views', './views');
...
app.get("/:backgammongame", (req, res) => {
    res.render("backgammon-game", {
        game_num: req.params.backgammongame
    })
});
//#endregion
...
io.on('connection', (socket) => {
    ...
    socket.on("GET-USER", (game_index, user_num)=>{ socket.emit("REC-USER", backgammon_games[game_index][`user${user_num}`]) });
    //#endregion
});
...
function game(user1, user2, game) {
    ...
    this.user1.set_cubes = (num1, num2) => {
        this.user1.cubes[this.cube_set] = [num1, num2];
    }
    this.user2.set_cubes = (num1, num2) => {
        this.user2.cubes[this.cube_set] = [num1, num2];
    }
    ...
    this.set_cubes = () => {
        ...
        let current_set = this.cube_set;
        this.role_cubes(1);
        this.cube_set = current_set;
        this.role_cubes(2);
    }
    this.role_cubes = (user_num) => {
        let num_cubes = 11 - this[`user${user_num}`].numD;
